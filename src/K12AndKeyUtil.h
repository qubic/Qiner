...
static uint64_t __shiftleft128 (uint64_t  LowPart, uint64_t HighPart, unsigned char Shift)
{
    uint64_t ret;
...
static uint64_t __shiftright128 (uint64_t  LowPart, uint64_t HighPart, unsigned char Shift)
{
    uint64_t ret;
...
{ // 256-bit Montgomery multiplication modulo the curve order, mc = ma*mb*r' mod order, where ma,mb,mc in [0, order-1]
    ...
    {
        memcpy(P, ma, 32);
        memset( ((unsigned char*)P) + 32, 0, 32);
    }
    else
    {
...
VOID_FUNC_DECL encode(point_t P, unsigned char* Pencoded)
{ // Encode point P
...
BOOL_FUNC_DECL getSharedKey(const unsigned char* privateKey, const unsigned char* publicKey, unsigned char* sharedKey)
